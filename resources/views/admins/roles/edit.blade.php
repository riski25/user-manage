                ...
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="name" id="name" placeholder="Nama" value="{{ $role->name}}" required>
                            @error('name')
                        ...
                        <div class="form-group">
                            <label for="">Multiple Permission for Role</label>
                             {{-- {{ dd($permission, $selectedPermissions->toArray()) }} --}}
                              {{-- {{ dd(in_array($permission, $role->permissions()->pluck('name')->toArray()) ? 'selected' : '')  }} --}}
                            <select class="form-control" name="permission[]" id="permission" required multiple>
                                @foreach ($permission as $data)
                                    <option value="{{ $data }}"
                                    {{in_array($data, $selectedPermissions->toArray()) ? 'selected' : ''}}
                                    >{{ $data }}</option>
                                @endforeach
                            </select>
                            @error('permission')
                        ...
                        <button type="submit" id="btn-data" class="btn btn-primary">Simpan Perubahan</button>
                    </form>
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        // console.log({{url('users/admin/role/'.$role->id)}})
        ...
        $('#btn-data').click('submit',function (e) {
            ...
            $.ajax({
                type: "PATCH",
                url: "{{ url('users/admin/role/'.$role->id) }}",
                data: {
                    "name" : nama,
